...
## Implementation overview and design review

The project was developed as a staged support-agent system, with each component handling a specific part of the overall workflow. A deterministic dataset was used to represent realistic loan records, while an authored knowledge base provides the policies and information needed to answer support-related questions. For semantic search, the system uses embeddings with ChromaDB, allowing relevant policy information to be retrieved based on the meaning of a user’s query rather than just matching keywords.

LangGraph is responsible for coordinating the agent’s workflow. Depending on the type of request, it routes the query either to the policy-retrieval process or to the loan-status lookup tool. The completed agent is then exposed through FastAPI, making it accessible as an HTTP service.

To keep the project reliable and reproducible, deterministic seeds and fixed test fixtures were used throughout the implementation. Local embeddings and `MOCK_LLM`-style scoring also allow the system to run without requiring paid API keys or relying on unpredictable external model generation. Two chunking strategies were considered for the knowledge base. Sentence-based chunking was selected as the recommended approach because it keeps individual policy statements clear and readable, while fixed-size chunking remains available for comparison and evaluation.

Security and reliability were also considered as part of the design. Before a request reaches the model, the system checks for personally identifiable information (PII) and potential prompt-injection attempts. Groundedness is then checked before the final response is returned to ensure that the answer is supported by the retrieved information. Request logging uses JSON Lines, with the same PII masking applied before any request text is stored.

The system also includes persistence and recovery features to make it more practical for real-world use. Conversation history is stored in JSON, while SQLite checkpoints allow a LangGraph conversation thread to resume after an interruption. Retry and timeout mechanisms help handle temporary failures and prevent slow or unresponsive operations from blocking the service. Finally, the MCP server is kept separate from the main LangGraph process, allowing the loan-lookup capability to be reused through a standard tool protocol.

Overall, the implementation combines retrieval, agent orchestration, tool use, security checks, persistence, and API serving into a single support-agent workflow that can be tested and run locally without depending on paid or unpredictable external services.
